...
public class LocaleUtil {
	...
	public static enum SupportedLanguageCodes {
		en, ta_IN
	};
	...
	public static String getLocalisedString(String key) {
		return getLocalisedString(key, null, BaseSession.getLocale());
	}

	public static String getLocalisedString(String key, Locale locale) {
		return getLocalisedString(key, null, locale);
	}
	...
	public static boolean isSupportedLanguage(String code) {
		return Stream.of(SupportedLanguageCodes.values()).filter(lang -> lang.name().equals(code)).findAny()
				.isPresent();
	}
