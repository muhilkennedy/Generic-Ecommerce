...
public class SecurityConfiguration { // implements WebMvcConfigurer {
    ...
    public PasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder(15);// Increase work factor for more security
    }
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		return http.csrf((csrf) -> csrf.disable())
				...
				.cors((cor) -> cor.configurationSource(corsConfigurationSource())).authorizeHttpRequests(config -> {
		...
		/*
		 * .oauth2Login(config -> { // incase of server side oauth verification
		 * }).build();
		 */
	}
