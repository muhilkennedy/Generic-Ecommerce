...
//	private AuditService auditService;
	 ...
	 * headers, HttpStatus status, WebRequest request) { String errorMessage =
	 * ex.getBindingResult().getFieldErrors().stream() .map(error ->
	 * String.format("%s %s", error.getField(),
	 * error.getDefaultMessage())).findFirst() .orElse(ex.getMessage());
	 * ErrorResponse errorResponse = new
	 * ErrorResponse(HttpStatus.BAD_REQUEST.value(), errorMessage);
	 * Log.base.error(errorResponse.toString()); if (logger.isDebugEnabled()) {
	 * ex.printStackTrace(); }
